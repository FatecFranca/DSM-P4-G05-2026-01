...
export type DashboardAnalytics = {
  ...
  mediana_hora: number;
  pct_pico: number;
  evolucao_hora: { gymName: string; hora: number; checkins: number }[];
  volume_dia: { gymName: string; dia_semana: string; checkins: number }[];
};

export async function getDashboardAnalytics(gymName?: string): Promise<DashboardAnalytics> {
  const url = gymName
